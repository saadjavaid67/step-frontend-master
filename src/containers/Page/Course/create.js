...
import { createCourse } from '../../../redux/request/actions';
...
function Page(props) {
  ...
  const createResponse = useSelector(state => state.createCourse);
  const saving = createResponse.isFetching;

  // Create Response
  const prevCreateLoading = usePrevious(createResponse.isFetching);
  useEffect(() => {
    handleApiResponse(prevCreateLoading, createResponse, '/app/course');
