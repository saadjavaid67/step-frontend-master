...
import { createUser } from '../../../redux/request/actions';
...
function Page(props) {
  ...
  const createResponse = useSelector(state => state.createUser);
  const saving = createResponse.isFetching;

  // Create Response
  const prevCreateLoading = usePrevious(createResponse.isFetching);
  useEffect(() => {
    handleApiResponse(prevCreateLoading, createResponse, '/app/user');
