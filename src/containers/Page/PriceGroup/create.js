...
import { createPriceGroup } from '../../../redux/request/actions';
...
function Page(props) {
  ...
  const createResponse = useSelector(state => state.createPriceGroup);
  const saving = createResponse.isFetching;

  // Create Response
  const prevCreateLoading = usePrevious(createResponse.isFetching);
  useEffect(() => {
    handleApiResponse(prevCreateLoading, createResponse, '/app/priceGroup');
  ...
  function handleSubmit(formValues) {
    ...
    dispatch(createPriceGroup({ data }));
  }
